...
- Adding a blog post: copy the **POST TEMPLATE** block at the bottom of `content/posts.ts`, fill it in, add it to the array, then generate its banner with `npm run generate:banner`. The banner is **required**; the postbuild gate fails the build without it. Route, sitemap entry, index row, search index, feed entry, and metadata all derive from the content layer, so there are no other files to touch.
- Draft posts (`status: "draft"`) never reach the site: `getPosts()` and `getPostBySlug()` return published posts only, so a draft has no route, index row, sitemap/search-index/feed entry, and nothing for crawlers to rank. Drafts live in `content/posts.ts` as future work; publishing (`status: "published"` + `publishedAt`) is Vasu's decision and puts a post live everywhere automatically. The "In draft" stamp in the post UI is retained but currently unreachable.
